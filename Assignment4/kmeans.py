...
def toyProblem():
  ...
  for i in range(50):
    SSE_rand.append(kMeansClustering(X, k=k, max_iters=max_iters, visualize=False)[2][-1])
  print("lensse: ", len(SSE_rand))
  ...
  for k in range(1, 150):
    SSE_vs_k.append(kMeansClustering(X, k=k, max_iters=max_iters, visualize=False)[2][-1])
...
def initalizeCentroids(dataset, k):
  return np.array([dataset[np.random.randint(0, len(dataset))] for i in range(k)])
...
def computeAssignments(dataset, centroids):
  dataPointsDistance = [np.linalg.norm(i - centroids, axis=1) for i in dataset]
  return np.array([np.argmin(i) for i in dataPointsDistance])[:,np.newaxis]
...
def updateCentroids(dataset, centroids, assignments):
  ...
  for i in range(centroids.shape[0]):
    ...
    counts.append(count)
  counts = np.array(counts)[:,np.newaxis]
...
def calculateSSE(dataset, centroids, assignments):
  ...
  for i in range(dataset.shape[0]):
    tot += np.square(np.linalg.norm(dataset[i] - centroids[assignments[i]]))
  return (tot)
