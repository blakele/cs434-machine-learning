...
number_of_layers = 8
width_of_layers = 512  # only matters if number of layers > 1
...
def main():
  ...
  for i in range(len(X_test)):
        test_predictions.append(np.argmax(net.forward(X_test[i])))
  ...
  with open(filename, 'w+', newline='') as csvfile:
    ...
    for i in range(len(test_predictions)):
          csvwriter.writerow([i, test_predictions[i]])        
...
class LinearLayer:
  ...
  def backward(self, grad):
    ...
    gradbias = np.sum(grad, axis=0)
    a = gradbias[:,np.newaxis].T
    
    self.grad_weights = self.input.T@grad
    self.grad_bias = np.sum(grad, axis=0)[:,np.newaxis].T
